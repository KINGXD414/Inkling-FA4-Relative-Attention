...
def _fa4_rel_attn_prefill_kernel(
    ...
    head_dim: tl.constexpr, rel_extent: tl.constexpr,
    BLOCK_Q: tl.constexpr, BLOCK_K: tl.constexpr,
    CAUSAL: tl.constexpr, WINDOW_LEFT: tl.constexpr,
):
    ...
    if q_tile_sz <= 0: return
    off_q = tl.arange(0, BLOCK_Q)
    off_d = tl.arange(0, head_dim)
    off_k = tl.arange(0, BLOCK_K)
    q_idx = q_start + q_off + off_q
    q_ptrs = q_ptr + q_idx[:, None] * stride_q_t + h * stride_q_h + off_d[None, :]
    q_mask = (off_q[:, None] < q_tile_sz) & (off_d[None, :] < head_dim)
    q = tl.load(q_ptrs, mask=q_mask, other=0.0).to(tl.float16)
    scale = tl.full([1, 1], 1.0 / tl.sqrt(head_dim), dtype=tl.float16)
    ...
    for start_k in range(0, k_len, BLOCK_K):
        ...
        if CAUSAL:
            mask = mask & ((q_off + off_q[:, None]) >= (start_k + off_k[None, :]))
        if WINDOW_LEFT >= 0:
            mask = mask & ((q_off + off_q[:, None]) - (start_k + off_k[None, :]) <= WINDOW_LEFT)
        k_ptrs = k_ptr + k_idx[None, :] * stride_k_t + kv_h * stride_k_h + off_d[:, None]
        k_mask = (off_k[None, :] < k_tile_sz) & (off_d[:, None] < head_dim)
        k = tl.load(k_ptrs, mask=k_mask, other=0.0).to(tl.float16)
        s = tl.dot(q, k).to(tl.float32) * (1.0 / head_dim)
        rel_dist = (q_off + off_q[:, None] + offset_seq) - (start_k + off_k[None, :])
        in_range = (rel_dist >= 0) & (rel_dist < rel_extent)
        rel_idx = tl.where(in_range, rel_dist, tl.zeros_like(rel_dist))
        rp = rel_ptr + (q_start + q_off + off_q[:, None]) * stride_rel_t + h * stride_rel_h + rel_idx
        rm = in_range & (off_q[:, None] < q_tile_sz) & (off_k[None, :] < k_tile_sz)
        rv = tl.load(rp, mask=rm, other=0.0).to(tl.float32)
        s = tl.where(in_range & mask, s + rv, s)
        ...
        acc = acc * alpha
        l_i = l_i * alpha
        v_ptrs = v_ptr + k_idx[:, None] * stride_v_t + kv_h * stride_v_h + off_d[None, :]
        v_mask = (off_k[:, None] < k_tile_sz) & (off_d[None, :] < head_dim)
        v = tl.load(v_ptrs, mask=v_mask, other=0.0).to(tl.float16)
        acc = acc + tl.dot(p.to(tl.float16), v).to(tl.float32)
        l_i = l_i + l_ij
        m_i = m_new
    acc = acc / (l_i + 1e-30)
    o_ptrs = out_ptr + q_idx[:, None] * stride_out_t + h * stride_out_h + off_d[None, :]
    o_mask = (off_q[:, None] < q_tile_sz) & (off_d[None, :] < head_dim)
