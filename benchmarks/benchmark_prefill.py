...
Usage:
    ...
    python benchmarks/benchmark_prefill.py --seq-len 512 --heads 8 --kv-heads 2

Output: latency (ms) and TFLOPs/s for each configuration.
"""
...
def benchmark_prefill(seq_len, n_heads, kv_heads, causal=True):
    """Measure kernel latency for a single prefill sequence."""
    total_q = seq_len
    total_k = seq_len
    ...
    # TFLOPs (attention: 2 * seq * seq * head_dim * n_heads per forward)
...
def main():
    ...
    for sl in seq_lens:
        try:
            ms, tflops = benchmark_prefill(sl, args.heads, args.kv_heads)
